...
#include "scout_mini_omni_msgs/ScoutStatus.h"
#include "scout_mini_omni_msgs/ScoutBmsStatus.h"
#include "scout_mini_omni_msgs/ScoutLightCmd.h"
...
namespace westonrobot
{
  ...
  ScoutROSMessenger::ScoutROSMessenger(ScoutMiniOmniRobot *scout, ros::NodeHandle *nh)
      : scout_(scout), nh_(nh) {}
  ...
  void ScoutROSMessenger::SetupSubscription()
  {
    ...
    status_publisher_ = nh_->advertise<scout_mini_omni_msgs::ScoutStatus>("/scout_status", 10);
    BMS_status_publisher_ = nh_->advertise<scout_mini_omni_msgs::ScoutBmsStatus>("/BMS_status", 10);

    // cmd subscriber
    motion_cmd_subscriber_ = nh_->subscribe<geometry_msgs::Twist>(
        "/cmd_vel", 5, &ScoutROSMessenger::TwistCmdCallback, this);
    light_cmd_subscriber_ = nh_->subscribe<scout_mini_omni_msgs::ScoutLightCmd>(
      ...
      const geometry_msgs::Twist::ConstPtr &msg)
  {
    if (!simulated_robot_)
    {
        scout_->SetMotionCommand(msg->linear.x, msg->angular.z, msg->linear.y);
    }
    else
    {
                                                    ...
                                                    double &lateral)
  {
    ...
    lateral = current_twist_.linear.y;
  }

  void ScoutROSMessenger::LightCmdCallback(
      const scout_mini_omni_msgs::ScoutLightCmd::ConstPtr &msg)
  {
    if (!simulated_robot_)
    {
      if (msg->enable_cmd_light_control)
      {
        ...
        {
          case scout_mini_omni_msgs::ScoutLightCmd::LIGHT_CONST_OFF:
          {
            cmd.front_light.mode = CONST_OFF;
            break;
          }
          case scout_mini_omni_msgs::ScoutLightCmd::LIGHT_CONST_ON:
          {
            cmd.front_light.mode = CONST_ON;
            break;
          }
          case scout_mini_omni_msgs::ScoutLightCmd::LIGHT_BREATH:
          {
            cmd.front_light.mode = BREATH;
            break;
          }
          case scout_mini_omni_msgs::ScoutLightCmd::LIGHT_CUSTOM:
          {
  ...
  void ScoutROSMessenger::PublishStateToROS()
  {
    ...
    scout_mini_omni_msgs::ScoutStatus status_msg;
    scout_mini_omni_msgs::ScoutBmsStatus bms_status;
...
//      bms_status.Warning_Status_2 = state.Warning_Status_2;
    ...
    PublishOdometryToROS(robot_state.motion_state.linear_velocity, robot_state.motion_state.angular_velocity,robot_state.motion_state.lateral_velocity, dt);

    // record time for next integration
    last_time_ = current_time_;
  }

  void ScoutROSMessenger::PublishSimStateToROS(double linear, double angular, double lateral)
  {
    current_time_ = ros::Time::now();
    ...
    status_msg.lateral_velocity = lateral;
    ...
    // publish odometry and tf
    PublishOdometryToROS(linear, angular, lateral, dt);
                                               ...
                                               double lateral,double dt)
  {
    ...
    double d_y = (linear_speed_ * std::sin(theta_) + lateral) * dt;
    ...
    odom_msg.twist.twist.linear.y = lateral;
